...
#define MPLAY_JUMP_TABLE_FUNC(n) (*(MPlayJumpTable + (n)))
...
void m4aSoundInit(void)
{
    ...
    SoundInit(&SoundInfo);
    MPlayExtender(CgbChans);
    m4aSoundMode(SOUND_MODE);

    for (i = 0; i < NUM_MUSIC_PLAYERS; i++)
    {
        struct MusicPlayer * music_player = MusicPlayerTable[i].music_player;
        MPlayOpen(music_player, MusicPlayerTable[i].track, MusicPlayerTable[i].num_tracks);
        music_player->unk_B = MusicPlayerTable[i].unk_A;
        music_player->mem_acc_area = MPlayMemAccArea;
    }
}
...
void m4aMPlayAllStop(void)
{
    ...
    for (i = 0; i < NUM_MUSIC_PLAYERS; i++)
        m4aMPlayStop(MusicPlayerTable[i].music_player);
}
...
void m4aMPlayAllContinue(void)
{
    ...
    for (i = 0; i < NUM_MUSIC_PLAYERS; i++)
        MPlayContinue(MusicPlayerTable[i].music_player);
}
...
void MPlayExtender(struct CgbChannel * cgb_channels)
{
    ...
    MPlayJumpTable[8] = ply_memacc;
    MPlayJumpTable[17] = ply_lfos;
    MPlayJumpTable[19] = ply_mod;
    MPlayJumpTable[28] = ply_xcmd;
    MPlayJumpTable[29] = ply_endtie;
    MPlayJumpTable[30] = SampleFreqSet;
    MPlayJumpTable[31] = TrackStop;
    MPlayJumpTable[32] = FadeOutBody;
    MPlayJumpTable[33] = TrkVolPitSet;
...
void SoundInit(struct SoundInfo * sound_info)
{
    ...
    MPlayJumpTableCopy(MPlayJumpTable);

    sound_info->mplay_jump_table = MPlayJumpTable;
            ...
            if (channels->envelope_counter == 0)
            {
                ...
                if ((channels->flags & SOUND_CHANNEL_SF_ENV) == SOUND_CHANNEL_SF_ENV_RELEASE)
                {
                    ...
                    envelope_pseudoecho_start:
                        channels->envelope_volume =
                            ((channels->envelope_goal * channels->pseudo_echo_volume) + 0xFF) >> 8;
...
{
    MPlayJumpTable[0](music_player, track);
}
...
MPlayFunc const gMPlayJumpTableTemplate[] = {
    ply_fine,    ply_goto,     ply_patt,       ply_pend,        ply_rept,  ply_fine,   ply_fine,      ply_fine,
    ply_fine,    ply_prio,     ply_tempo,      ply_keysh,       ply_voice, ply_vol,    ply_pan,       ply_bend,
    ply_bendr,   ply_lfos,     ply_lfodl,      ply_mod,         ply_modt,  ply_fine,   ply_fine,      ply_tune,
    ply_fine,    ply_fine,     ply_fine,       ply_port,        ply_fine,  ply_endtie, SampleFreqSet, TrackStop,
    FadeOutBody, TrkVolPitSet, RealClearChain, RealClear64byte,
};
